...
// New Underwriting — the supply-side submit form. A phone can't scrape RPR, so
// this always runs source 'mobile_app': served by an RPR agent or QUEUED until one
// is live (the user is told, and gets a push when it's ready).
...
export default function NewUnderwriting() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Running now — open the report (it shows live progress, then the full report).
      router.replace({
